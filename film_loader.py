...
def transform(__actors, __writers, __raw_data):
    ...
    for movie_info in __raw_data:
        ...
        movie_id, imdb_rating, genre, title, description, director, raw_actors, raw_writers = (
            movie_info
        )
        ...
        writers_list = [
            (writer_id, __writers.get(writer_id))
            for writer_id in new_writers.split(',')
        ]
        actors_list = [
            (actor_id, __actors.get(int(actor_id)))
        ...
        document = {
            ...
            'director': director,
            'actors': [
                {'id': actor[0], 'name': actor[1]}
                for actor in set(actors_list)
                if actor[1]
            ],
            'writers': [
                {'id': writer[0], 'name': writer[1]}
                for writer in set(writers_list)
                if writer[1]
            ],
        }
        ...
        document['actors_names'] = (
            ', '.join([actor['name'] for actor in document['actors'] if actor])
            or None
        )
        document['writers_names'] = (
            ', '.join(
                [writer['name'] for writer in document['writers'] if writer]
            )
            or None
        )
